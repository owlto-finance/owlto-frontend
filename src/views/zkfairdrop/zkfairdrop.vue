<template>
  <div class="deploy">
    <div class="title">ZKFair Airdrop Batch Query</div>
    <div class="deploy-query-box">
      <div class="deploy-query">
        ...
        <div class="deploy-query-result">
          <div class="head">
            <span>Address</span>
            <span> Airdrops (<span style="color: #f4c237">$ZKF</span>) </span>
          </div>
          <div class="content">
            <div
              class="item"
            ...
            >
              ...
              <div class="item-pass">
                <div>
                  <div class="label">Lumoz:</div>
                  <div class="value">
                    {{ Number(item.Lumoz.value_decimal).toFixed(2) }}
                  </div>
                </div>

                <div>
                  <div class="label">zkEVM:</div>
                  <div class="value">
                    {{ Number(item["Polygon zkEVM"].value_decimal).toFixed(2) }}
                  </div>
                </div>

                <div>
                  <div class="label">ZK-L2s:</div>
                  <div class="value">
                    {{ Number(item.zkRollups.value_decimal).toFixed(2) }}
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="deploy-buttons">
          <div class="value-box" @click="bridge">
            <div style="display: flex; justify-content: space-between">
              <span
                >ZKFair uses USDC as gas fees. There is no USDC on
                ZKFair?&nbsp;&nbsp;&nbsp;&nbsp;</span
              >
              <span>Bridge via Owlto now →</span>
            </div>
...
const bridge = () => {
  ...
  window.open("https://owlto.finance/bridge?to=ZKFair", "_blank");
};
  ...
  try {
    ...
    for (let i = 0, len = addressList.length; i < len; i++) {
      const { data } = await request3({
        ...
        url: "api/lottery/maker/zkfair/drop",
        data: {
