        ...
        <div class="left_box">
          <div class="table">
            <div class="th">
              ...
              <div class="th_item">
                Totals
                <PopoverImg show-text="Only ETH commissions are currently displayed." />
              </div>
            </div>
            <n-space vertical>
              <n-spin :show="commissionsLoading" stroke="#f5c539">
                <div class="scroll" v-if="commissionData.length > 0">
                  <div :class="['td', index % 2 == 0 ? '' : 'bg_white']" v-for="(item, index) in commissionData">
                    <div class="td_rank">{{ index + 1 }}</div>
                ...
                <div class="no_data" v-else>
                  <van-empty image-size="7rem" description="No Data" class="show" />
                </div>
              </n-spin>
            </n-space>
          </div>
            ...
            <n-space vertical>
              <n-spin :show="pointsLoading" stroke="#f5c539">
                <div class="scroll" v-if="pointsData.length > 0">
                  <div :class="['td', index % 2 == 0 ? '' : 'bg_white']" v-for="(item, index) in pointsData">
                    <div class="td_rank">{{ index + 1 }}</div>
                    <div class="td_address">{{ ellipsis(item.user) }}</div>
...
const getCommissionData = async () => {
  try {
    ...
    const res = await request({
      url: "reward/commissions-rank-list",
    });
